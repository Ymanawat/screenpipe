...
const config = {
  ...
  windows: {
    ...
    ffmpegUrl:
      "https://unlimited.dl.sourceforge.net/project/avbuild/windows-desktop/ffmpeg-7.0-windows-desktop-vs2022-default.7z?viasf=1",
...
if (platform == "windows") {
  ...
  if (!(await fs.exists(config.ffmpegRealname))) {
    ...
    // Use the extract7z helper function instead of direct command
    const extracted = await extract7z(
    ...
    if (!extracted) {
      console.error(
        "Failed to extract FFMPEG archive. Please install 7-Zip and try again."
      );
      process.exit(1);
    }
  ...
  if (!(await fs.exists(onnxRuntimeName))) {
    ...
    try {
      ...
      // Use 7z instead of unzip
      const extracted = await extract7z(onnxRuntimeLibs, ".");
      if (!extracted) {
        throw new Error("Failed to extract ONNX Runtime archive");
      }
  ...
  await $`SystemDrive=${process.env.SYSTEMDRIVE} SystemRoot=${process.env.SYSTEMROOT} windir=${process.env.WINDIR} C:\\packages\\vcpkg\\vcpkg.exe install ${config.windows.vcpkgPackages}`.quiet();
}
  ...
  for (const arch of architectures) {
    ...
    if (arch === "arm64") {
      ...
      try {
        ...
        if (
          (await fs.exists("screenpipe-aarch64-apple-darwin")) &&
          !isDevMode
        ) {
...
async function installOllamaSidecar() {
  ...
  if (
    (platform === "macos" &&
      (await fs.exists(path.join(ollamaDir, "ollama-aarch64-apple-darwin"))) &&
  ...
  try {
    ...
    if (platform === "windows") {
      ...
      await fs.rename(
        ...
        path.join(ollamaDir, ollamaExe)
      );
    } else if (platform === "linux") {
      await $`tar -xzf "${downloadPath}" -C "${ollamaDir}"`;
      await fs.rename(
        path.join(ollamaDir, "bin/ollama"),
        path.join(ollamaDir, ollamaExe)
      );
    } else if (platform === "macos") {
      ...
      await fs.copyFile(
        downloadPath,
        path.join(ollamaDir, "ollama-aarch64-apple-darwin")
      );
      await fs.copyFile(
        downloadPath,
        path.join(ollamaDir, "ollama-x86_64-apple-darwin")
      );
    }
...
async function extract7z(archivePath, destinationPath) {
  const possiblePaths = [
    "C:\\Program Files\\7-Zip\\7z.exe",
    "C:\\Program Files (x86)\\7-Zip\\7z.exe",
    "7z", // Will work if 7z is in PATH
  ];

  for (const sevenZipPath of possiblePaths) {
    try {
      await $`${sevenZipPath} x ${archivePath} -o${destinationPath} -y`.quiet();
      console.log(
        `Successfully extracted ${archivePath} using ${sevenZipPath}`
      );
      return true;
    } catch (error) {
      console.warn(`Failed to extract using ${sevenZipPath}: ${error}`);
      continue;
    }
  }

  console.error(
    "Could not find 7-Zip. Please install 7-Zip and make sure it's in your PATH"
  );
  return false;
}
